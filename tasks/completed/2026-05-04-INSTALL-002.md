# Tâche : INSTALL-002 — `arc setup` exécution stack

## Statut final
✅ Terminée — 2026-05-04
Durée réelle : ~3h30 (≈ +25% vs estim 2h45)
PR : merge direct sur `main` (8 commits — 7 sous-tâches + 1 fix de design)

## Bilan

### Ce qui a bien marché
- Cadrage UX/transactionnel figé en début de sous-tâche 4 a évité les débats en review.
- Détection du défaut de design rename/ansible AVANT que ANSIBLE-001 commence (gain ~2h évité plus tard).
- 7 décisions UX de la sous-tâche 5 + cadrage transactionnel de la 4 = aucune ambiguïté en review.
- Smoke test manuel validé en conditions réelles (vrai terminal, pas juste mocks).

### Ce qui a posé problème
- Cadrage initial avec des noms de composes inventés (`ai-stack-compose.yml`) qui ne matchaient pas le repo réel. Claude a bien fait de signaler. **Lesson** : toujours vérifier les noms d'artefacts existants avant de les inscrire dans un cadrage.
- Le défaut de design rename/ansible aurait pu être anticipé dans le cadrage initial — ce sera la prochaine fois.

### Lessons learned
- Les notes "à revisiter en TASK-X" sont parfois des défauts à traiter immédiatement, pas du parking. **Discipline** : challenger systématiquement chaque "parking" en demandant "est-ce vraiment parking ou est-ce un défaut actif ?".
- Le pattern transaction "tout ou rien sur le commit marker" (ici `state.json`) est plus utile en pratique que la transaction atomique pure : permet l'inspection en cas d'échec.
- Le smoke test manuel reste essentiel même quand les E2E mockés sont verts. Il valide ce que les mocks ne peuvent pas (UX, couleurs, comportement Ctrl+C, vraies interactions stdin).

### Décisions structurantes prises
- **Aucun nouvel ADR** (ADR-0011/0012/0014/0015 couvraient déjà les contraintes).
- **Convention de nommage actée** : composes = `docker-compose.{prod,agents,sandbox}.yml` (cf. correction du cadrage initial).
- **Contrat transactionnel `applyStack`** : "tout ou rien sur `state.json`", pas "tout ou rien sur disque".

### CLI gaps notés pour le futur
- `arc setup --reconfigure` (mentionné dans la doc mais pas livré).
- `arc setup --skip-dns` (référencé dans `install-without-public-ip.md`, attendait INSTALL-001/002 livrées — maintenant possible).
- **CLI-025** bundling `bun --compile` pour le playbook (parking propre, pas urgent).

### Métriques
- **Tests** : 107 → **140** (+33, +2 skipIf win32)
- **Fichiers** : 8 nouveaux + 4 modifiés
- **Commits** : 8 dont 1 `fix:` de design en cours de route
- **Effort** : 2h45 prévues → ~3h30 réelles (≈ +25%)

### Prochaine tâche suggérée
- **ANSIBLE-001** : rôles Ansible localhost (hardening, docker, coolify, ai-stack, sandbox, backups).
- Effort estimé : 3-5h sur 2 sessions.
- Dépendances : aucune bloquante (le stub playbook est en place, ANSIBLE-001 le remplace).
- **Recommandation** : NE PAS attaquer immédiatement. Faire une pause ≥ quelques heures, idéalement à la prochaine session avec tête fraîche.
