# DIST-001 — Distribution & packaging ARC

## Statut

✅ **Terminée — 2026-05-12** (démarrée le 2026-05-08, 4 jours actifs)
...
### CLI gaps notés (à traiter au moment opportun)
- ✅ **`arc.euglowlabs.com` DNS parking effectué le 2026-05-11** (record A → 192.0.2.1, RFC 5737, DNS only) — voir 1d-3 dans le rapport de completion ci-dessous.
...
### 2026-05-12 — 1f-A livré + DIST-001 clôturé

**Smoke E2E install validé in-vivo sur VMware Ubuntu 24.04 (utilisateur Johann)** :

```text
$ curl -fsSL https://install-arc.euglowlabs.com | ARC_VERSION=0.1.0-rc.1 sh
→ Installing arc v0.1.0-rc.1 (linux/x64)...
→ Downloading https://github.com/.../v0.1.0-rc.1/arc-linux-x64...
→ Downloading https://github.com/.../v0.1.0-rc.1/arc-linux-x64.sha256...
✓ SHA256 verified (300e706f124271657541e9d2aa055246d33959c52de2a9e37b5200ee1e2342f0)
[sudo: authenticate] Password:
✓ arc v0.1.0-rc.1 installed at /usr/local/bin/arc.

$ arc version
arc 0.1.0-rc.1 (sha=d125942, built=2026-05-12T18:59:43.659Z)

$ arc --help
[bannière + 14 commandes listées correctement]
```

Tous les critères d'acceptance DIST-001 sont satisfaits sur le scope strict "distribution & packaging". ✅

**Décision : v0.1.0 stable NON taggé** — `v0.1.0-rc.1` reste la dernière release publique tant que CLI-029 (auto-bootstrap) n'est pas livré. Raison : un utilisateur qui ferait `curl install-arc | sh` sans `ARC_VERSION` pin atterrirait sur v0.1.0 stable, puis verrait `arc setup --apply` planter sur l'absence d'Ansible — mauvaise première impression. Avec v0.1.0-rc.1 en prerelease, GitHub ne propose pas la "Latest" → l'API `/releases/latest` retourne 404 → l'installeur affiche une erreur claire qui invite à pinner explicitement la version, et les early adopters comprennent qu'ils sont en RC.

**Gap UX révélé pendant tentative 1f-B et déplacé en backlog** :

`arc setup --apply` suppose actuellement `ansible-playbook`, `python3`, `docker`, `sudo` pré-installés sur la machine cible. Sur une VM Ubuntu 24.04 fresh, Ansible n'est pas installé par défaut → contredit ADR-0011 A3 ("sans intervention manuelle entre les questions").

→ Nouveau ticket : **`tasks/backlog/CLI-029-auto-bootstrap-system-prerequisites.md`** — auto-installation des prérequis système avec consentement utilisateur unique, débloque tag v0.1.0 stable et smoke 1f-B.

### Rapport de completion DIST-001

**Périmètre livré** :

| Sous-tâche | Statut | Commit principal |
|---|---|---|
| 1a-bis (ADR-0016 distribution strategy) | ✅ | initial |
| 1a-1 (spike Bun embed) | ✅ | spike validé |
| 1a-2 (codegen + EmbeddedPlaybooksLoader) | ✅ | `fa9b021` + `1cad8db` |
| 1a-3 (`bun --define` version metadata) | ✅ | `ed59f27` |
| 1a-cleanup (URL pivot install.euglowlabs → install-arc) | ✅ | `773ba6a` |
| 1a-4 (stub `arc upgrade`) | ✅ | `0876e8c` |
| 1a-5 (smoke binaire compilé E2E local) | ✅ | `73c3458` |
| 1b (install.sh POSIX rewrite) | ✅ | `994237b` |
| 1c (CI publish.yml v2 tag-triggered) | ✅ | `366a0cf` |
| 1d-1 (`gen-install-page` + `dist/install/_headers`) | ✅ | `9df426e` + `08d3c04` |
| 1d-2 (Cloudflare Pages + custom domain) | ✅ | trace `1c03d91` (prod live) |
| 1d-3 (DNS parking `arc.euglowlabs.com` → 192.0.2.1) | ✅ | trace `1c03d91` |
| 1e-1 (5 docs : installation.md, release-process.md, +3 modif) | ✅ | `11b2c3d` |
| 1e-2 (sweep URL legacy 6 fichiers) | ✅ | `dc58e58` |
| 1f-A (smoke install pipeline sur VM utilisateur) | ✅ | in-vivo VMware Ubuntu 24.04 |
| 1f-B (smoke `arc setup --apply` complet) | ⏭ → CLI-029 | deferred (gap auto-bootstrap) |

**Commits totaux** : ~12 commits étiquetés `[DIST-001]`.

**Artefacts en prod** :
- Cloudflare Pages projet `euglowlabs-arc` connecté à `main` branch
- Custom domain `install-arc.euglowlabs.com` Active (HTTP/2 200, text/plain, max-age=300)
- DNS parking `arc.euglowlabs.com` → 192.0.2.1 (réservé Chantier 2)
- GitHub Release `v0.1.0-rc.1` (prerelease, 4 artefacts uploadés)
- Workflow `.github/workflows/publish.yml` opérationnel

**Documentation livrée** :
- `docs/installation.md` (152 lignes, user-facing)
- `docs/release-process.md` (137 lignes, SOP maintainer)
- `docs/03-architecture-decisions/0016-distribution-strategy.md`
- 8 fichiers existants mis à jour (URL migration + restructure quickstart)

**Apprentissages clés** :
- Cloudflare Pages v3 lit `.tool-versions` via asdf → ne supporte pas `pnpm` plugin → fix via `packageManager` field package.json (commit `08d3c04`)
- Gitignore : `dist/` exclut le dossier lui-même → impossible de re-include via `!dist/install/_headers`. Fix : `dist/*` + carve-out (commit `9df426e`)
- Le pipeline `install.sh source → gen → Pages → install-arc.euglowlabs.com` exige le repo public pour `curl` anonyme → repo basculé public le 2026-05-12 (cohérent ADR-0006)
- Gap UX `arc setup --apply` sans bootstrap système → décharge dans CLI-029 backlog

**Prochaines étapes recommandées** :
1. **CLI-029** (auto-bootstrap) — HIGH priorité, débloque tag stable v0.1.0
2. Puis **LOCAL-001** (mode WSL/laptop) — étend le marché cible
3. Audit Phase 1 ✅ archivées (post-mortem CLI-025/026/027 — pipeline pas bouclé à l'époque)
